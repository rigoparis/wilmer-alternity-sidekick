...
func _init() -> void:
	...
	rules.set_achievement_points(character, 30)
	...
	var check := rules.can_purchase_achievement(character, increase)
	if not bool(check.get("allowed", false)):
		_fail("Action Check Increase should be purchasable at Combat Spec level 5.")
	var result := rules.add_achievement_purchase(character, "action_check_increase")
	...
	if rules.achievement_points_spent(character) != 4:
	...
	result = rules.add_achievement_purchase(character, "wil_increase_1")
	if not bool(result.get("ok", false)):
	...
	rules.set_achievement_points(character, 51)
	character["profession_id"] = 0
	result = rules.add_achievement_purchase(character, "remove_flaw", "obsessed", 4)
	if not bool(result.get("ok", false)):
	...
	if rules.achievement_points_spent(character) < 22:
